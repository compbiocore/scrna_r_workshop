...
GSEA is a more in-depth method that can capture coordinated changes in pathways even if the differences are small or some of the genes are not differentially expressed. We won't get into the details for the method here, but the three important results from the output to keep in mind are:
...
In order to do GSEA, we have to created a ranked list of genes. How to do so is an open question. Additionally the algorithm for GSEA is meant for bulk RNAseq and comparing between groups, and is not directly applicable to single cell RNAseq which also includes variability in clusters of cell types. For those reasons we would not necessarily recommend running GSEA, but here are some suggestions for if it is something you want to do.

We will use the package `fgsea` to do a GSEA analysis. Suggested on issue#50 in the fgsea repo is using either one of two metrics to rank the genes: avg_logFC, or -sign(avg_log2FC)*log10(p_val_adj). In experiments we've done, `avg_logFC` tends to return no significant pathways, while `-sign(avg_log2FC)log10(p_val_adj)` returns more significant pathways but results that are somewhat unexpected. `fgsea` takes as input a vector of the scores where the names are the gene symbols, and a corresponding list of gene sets where each entry has the list of gene symbols comprising that gene set.

```{r}
library(fgsea)

ranks <- ctl_vs_stim$avg_log2FC # replace with -sign(avg_log2FC)*log10(p_val_adj) if ranking other way
ranks[ranks==Inf] <- 500000 # FTL is Inf which cannot be input to fgsea
names(ranks) <- rownames(ctl_vs_stim)
ranks[1:10]

gene_sets <- split(filtered_genes$gene_symbol, f = filtered_genes$gs_name)
gene_sets

res <- fgsea(pathways=gene_sets, stats=ranks,minSize=10,maxSize=500)
res$ID <- 1:nrow(res)
res
```

As we can see, none of the pathways are considered significant. We can rerun this code using `-sign(avg_log2FC)log10(p_val_adj)` instead:

```{r}
ranks <- -sign(ctl_vs_stim$avg_log2FC)*log10(ctl_vs_stim$p_val_adj)
ranks[ranks==Inf] <- 500000 # FTL is Inf which cannot be input to fgsea
names(ranks) <- rownames(ctl_vs_stim)
ranks[1:10]
...
None of the pathways are deemed significant here still, but the interferon signaling pathway has at least a pvalue closer to 0.05.

We can also do bubble plots with these results:
```{r}
library(ggrepel)
ggplot(res, aes(x=ID,y=-log10(padj), color=NES, label=pathway)) +
    geom_point(shape=19,aes(fill=NES,size=size), alpha=0.8) +
    scale_color_gradient2() + scale_fill_gradient2() + geom_label_repel(nudge_y=0.05)
```

### iDEA

GSEA and ORA are dependent on an initial differential expression analysis. However, the differential expression of an individual gene also has an obvious dependence on gene set enrichment, as gene sets contain information about the individual genes within the set. iDEA (integrative Differental expression and gene set Enrichment Analysis) addresses this joint dependence through a model that jointly estimates both differential expression and gene set enrichment. (https://xzhoulab.github.io/iDEA/) The input to this method is still a list of per-gene summary statistics from an initial differential expression analysis, but iDEA has increased power and provides updated results for detection of both differentially expressed genes and gene sets.

iDEA expects an input of gene-level summary stats -- the fold changes and their variances. Here we can calculate that information from our Seurat output:
...
```{r}
idea <- CreateiDEAObject(as.data.frame(sum_stats_sub), as.data.frame(annots_sub), num_core=4)
```
